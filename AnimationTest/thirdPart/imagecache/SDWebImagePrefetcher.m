...
- (void)startPrefetchingAtIndex:(NSUInteger)index
{
    ...
    [self.manager downloadWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished)
    {
        ...
        if (image)
        {
            NSLog(@"Prefetched %ld out of %ld", self.finishedCount, self.prefetchURLs.count);
        }
        else
        {
            NSLog(@"Prefetched %ld out of %ld (Failed)", self.finishedCount, [self.prefetchURLs count]);
...
- (void)reportStatus
{
    ...
    NSLog(@"Finished prefetching (%ld successful, %ld skipped, timeElasped %.2f)", total - self.skippedCount, self.skippedCount, CFAbsoluteTimeGetCurrent() - self.startedTime);
}
