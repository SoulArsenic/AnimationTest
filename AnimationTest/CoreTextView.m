...
CGPoint CGcorePointFlipped(CGPoint point, CGRect bounds)
{
	return CGPointMake(point.x, CGRectGetMaxY(bounds) - point.y);
}

CGRect CGcoreRectFlipped(CGRect rect, CGRect bounds)
{
        ...
        @autoreleasepool {
            ...
            for (int i = 0; i < CFArrayGetCount(lines); i++) {
                ...
                for (int j = 0; j < CFArrayGetCount(runs); j++) {
                    ...
                    __unused  CFRange range = CTRunGetStringRange(run);
...
//    CGPoint mirrorPoint = CGcorePointFlipped(point, self.bounds);
//    
//    CFArrayRef lines = CTFrameGetLines(textFrame);
//    CGPoint origins[CFArrayGetCount(lines)];
//    CTFrameGetLineOrigins(textFrame, CFRangeMake(0, 0), origins);
