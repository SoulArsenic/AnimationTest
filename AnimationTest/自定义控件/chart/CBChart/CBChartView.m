...
- (void) setOther:(NSArray *)other{
    ...
    if (self.leftLineMargin< (maxStrWidth + 6))
        self.leftLineMargin = maxStrWidth + 6;
...
-(void)setUpCoordinateSystem // 利用UIView作为坐标轴动态画出坐标系
{
    ...
    [[self viewWithTag:4499] removeFromSuperview];
    [[self viewWithTag:4500] removeFromSuperview];

    
    UIView *xCoordinate = [self getLineCoor];
    UIView *yCoordinate = [self getLineCoor];
    
    xCoordinate.tag = 4499;
    yCoordinate.tag = 4500;
...
//    [@"0" drawInRect:myRect withAttributes:self.textStyleDict];
    ...
    [self setUpYcoorWithValues:[self getMaxArray]];
    ...
    // 画曲线
    [self drawFuncLine:self.yValues andLayer:[self setUpLineLayer:_chartColor]];
    
    if (_other) {
        [self drawFuncLine:_other andLayer:[self setUpLineLayer:[UIColor cyanColor]]];
    }
}
...
- (NSArray *) getMaxArray{
    ...
    float maxTempY = 0;
    for (NSString * temp  in _yValues) {
        if ([temp floatValue] >= maxTempY) {
            maxTempY = [temp floatValue];
        }
    }
    
    float maxTempO = 0;
    for (NSString * temp  in _other) {
        if ([temp floatValue] >= maxTempO) {
            maxTempO = [temp floatValue];
        }
    }
    
    if (maxTempO > maxTempY) {
        return _other;
    }
    return _yValues;
}

-(void)drawFuncLine:(NSArray *) yValuesArray andLayer:(CAShapeLayer *) lineLayer
{
    if (self.xValues.count != 0 && yValuesArray.count != 0) {
        NSMutableArray *funcPoints = [NSMutableArray array];
        NSInteger pointCount = self.xValues.count;
        [[UIColor clearColor] set];
        
        if (self.xValues.count != yValuesArray.count) {
            pointCount = (self.xValues.count < yValuesArray.count ? self.xValues.count : yValuesArray.count);
        }
        ...
        for (int i = 0; i < pointCount; i++) {
            ...
            CGFloat yValue = [yValuesArray[i] floatValue];
...
-(CAShapeLayer *)setUpLineLayer:(UIColor *) color
{
...
//    transition.type = @"rippleEffect";
    ...
    transition.timingFunction = [CAMediaTimingFunction functionWithControlPoints: 0.292 : 0.750 : 0.569 : 0.028];
